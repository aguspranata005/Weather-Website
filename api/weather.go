...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	apiURL := fmt.Sprintf("%s?lat=%s&lon=%s&appid=%s&units=metric&lang=id", forecastAPIURL, lat, lon, apiKey)
	resp, err := http.Get(apiURL)
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusServiceUnavailable)
		json.NewEncoder(w).Encode(map[string]string{"error": "Gagal terhubung ke layanan cuaca"})
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			"error":   "Error dari layanan cuaca",
			"details": string(body),
		})
		return
	}
